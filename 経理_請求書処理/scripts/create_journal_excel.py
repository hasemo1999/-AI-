...
def create_journal_excel(data, output_path):
    ...
    # ヘッダー
    headers = ['日付', '借方勘定科目', '借方金額', '貸方勘定科目', '貸方金額',
               '税区分', '摘要', '取引先', '請求書番号', '判定']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', required=True, help='JSON file with journal data')
    parser.add_argument('--output', default='仕訳一覧.xlsx', help='Output Excel path')
    parser.add_argument('--csv', default='mf_import.csv', help='Output CSV path')
